...
After configuring the Splunk Universal Forwarder, it is important to verify that Windows Event Logs and Sysmon events are successfully reaching the Splunk Enterprise server. This validation confirms that the endpoint, forwarder, and SIEM are communicating correctly before performing attack simulations.

### Verify Windows Event Logs

Open the Splunk Search & Reporting application and execute the following search:

```spl
index=main
```

Successful results should display Windows event logs forwarded from the Windows endpoint.

Verify that the following log sources are available:
...
To verify that Sysmon events are being collected successfully, run the following search:

```spl
index=main source="WinEventLog:Microsoft-Windows-Sysmon/Operational"
```

The search should return Sysmon events generated by the Windows endpoint.

To verify a specific event type, search using an Event ID. For example:

```spl
index=main source="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventCode=3
```

This query returns Sysmon Network Connection events (Event ID 3), confirming that detailed endpoint telemetry is successfully forwarded to Splunk Enterprise.

### Validation Checklist

Verify the following before proceeding with attack simulations:

- Splunk Enterprise is running.
- Splunk Universal Forwarder service is running.
- Windows Event Logs are visible in Splunk.
- Sysmon Operational events are visible in Splunk.
- Searches return recent events without errors.

### Screenshot

<p align="center">
  <img src="screenshots/verify-log-collection.png" alt="Verify Log Collection" width="900">
</p>

**Figure 10:** Splunk Enterprise displaying indexed Sysmon Operational events collected from the Windows endpoint, confirming successful log collection and communication between the Splunk Universal Forwarder and the SIEM.
